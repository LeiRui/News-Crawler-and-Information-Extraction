#coding:utf8
'''
第三步：将task3_step2_out.pkl输入到提前训练好的分类模型中，模型输出得到实体关系分类结果，并将结果保存到一个xml文件中。
...
model = torch.load('ChineseNRE/model_trained/model_01.pkl') #提供一个提前训练好的模型

#模型的输入
with open('task3_step2_out.pkl', 'rb') as inp:
...
#类别id到关系名的映射
id2relation = {}
with codecs.open('ChineseNRE/data/people-relation/relation2id.txt','r','utf-8') as input_data:
    for line in input_data.readlines():
        id2relation[int(line.split()[1])] = line.split()[0]
    input_data.close()

BATCH=128 #注意这个数值使用和模型训练时一致的数值，因为BATCH会直接影响模型的hidden结构参数
remains=len(test)%BATCH

# 去掉余数变成batch的整数倍条的输入的处理
...
for sentence,pos1,pos2 in test_dataloader_mod:
    sentence = Variable(sentence) #已转换成字向量
    pos1 = Variable(pos1) 
    pos2 = Variable(pos2) 
    y = model(sentence,pos1,pos2)
